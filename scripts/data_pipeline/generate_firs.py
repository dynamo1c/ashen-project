...
def main():
    ...
    seed_path = os.path.join(ROOT_DIR, 'data', 'processed', 'bengaluru_crime_seed.json')
    ...
    # 8. Export directly to output files in data/seeds
    print("Writing output CSV files to data/seeds/...")
    fir_df.to_csv(os.path.join(SEEDS_DIR, 'fir_records_seed.csv'), index=False)
    print("  Saved 'fir_records_seed.csv' containing 75,000 incident transactions.")
    
    offenders_df.to_csv(os.path.join(SEEDS_DIR, 'offenders_seed.csv'), index=False)
    print(f"  Saved 'offenders_seed.csv' containing {len(offenders_df)} suspect-incident records.")
    
    drs_df.to_csv(os.path.join(SEEDS_DIR, 'district_risk_scores_seed.csv'), index=False)
